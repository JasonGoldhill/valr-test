...
def test_getOrderbook():
    assert orderbook.getOrderbook("BTCZAR") == {"Asks": [{"quantity": 1, "price": 75}, {"quantity": 0.5, "price": 100}, {"quantity": 1, "price": 200}], "Bids": [{"quantity": 1, "price": 50}]}

def test_getTradeHistory():
    assert orderbook.getTradeHistory("BTCZAR", 100) == {"TradeHistory": [{"quantity": 1, "price": 100, "takerSide": "sell"}, {"quantity": 0.5, "price": 150, "takerSide": "sell"}, {"quantity": 0.5, "price": 150, "takerSide": "sell"}]}
    assert orderbook.getTradeHistory("BTCZAR", 1) == {"TradeHistory": [{"quantity": 1, "price": 100, "takerSide": "sell"}]}